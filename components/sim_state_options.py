...
_NO_DEFAULT_VALUE = "_NO_DEFAULT_VALUE"

class StateOption(object):
    '''
    Describes the state option
    '''
    ...
    def __init__(self, name, types, default=_NO_DEFAULT_VALUE, description=None):
        ...
        if not isinstance(self.default, tuple(self.types)):
            raise SimStateOptionsError("The type of the default value does not match the expected types of the state option.")
...
class SimStateOptions(object):
    '''
    A per-state manager of state options. An option can be
    either a key-valued entry or seen as a key_valued entry
    whose value can only be either True or False
    '''
    ...
    def __init__(self, thing):
        '''
        :param thing: Either a set of Boolean switches to enable
        or an existing SimStateOptions instance.
        '''
    ...
    def __contains__(self, key):
        '''
        [COMPATIBILITY]
        In order to be compatible with the old interface,
        __contains__() only supports testing the value of
        boolean switch.
