...
class ForwardAnalysis(object):
    '''
    Well my attempt to build a static forward analysis framework that can serve as the base of multiple
    static analyses in quartz, including CFG analysis.

    In short, ForwardAnalysis performs a forward data-flow analysis by tranversing a graph, compute on abstract values,
    and store results in abstract states.
    '''

    def __init__(self, order_jobs=False, allow_merging=False, allow_widening=False, status_callback=None, graph_visitor=None):
        '''
        Constructor
        ...
        :paraam graph_visitor: A graph_visitor to provide succesors.
        ...
        if self._allow_widening and not self._allow_merging:
            raise QrtzForwardAnalysisError('Merging must be allowed if widening is allowed.')
        ...
        # A map between key to job. Jobs with the same key will be merged by calling _merge_jobs()
        self._job_map = {}
    ...
    def _merge_state_from_predecessors(self, node):
        ...
        return functools.reduce(lambda s0, s1: self._merge_states(node, s0, s1), states[1:], states[0])
    ...
    def _analysis_core_baremetal(self):
        ...
        while not self.should_abort:
            ...
            except QrtzDelayJobNotice:
                # delay the handling of this job
                continue
            except QrtzSkipJobNotice:
            ...
            if key in self._job_map:
                ...
                if self._allow_widening and self._should_widen_jobs(job_info.job, job):
                    try:
                        ...
                        job_added = True
                    except QrtzJobWideningFailureNotice:
                        # widening failed
                ...
                if not job_added:
                    ...
                    except QrtzJobMergingFailureNotice:
                        # merging failed
